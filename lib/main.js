...
const PATCHED_EDITOR_PROPERTIES = [
  "cursorIndex",
  "cursorPower",
  "cursorDecoration",
  "getSuperCursor",
...
module.exports = {
  ...
  activate() {
    ...
    this.disposables.add(
      lumine.commands.add("lumine-workspace", {
        "cursor-leader:power-global": {
          description: "Turn leader mode on or off in every editor at once.",
          didDispatch: () => {
            this.powerGlobal();
          },
        },
        // The three menu commands: the application menu dispatches at whatever
        // holds focus, so on the editor scope they were dead off-editor. Each
        // resolves the editor from the dispatch target, or the active one.
        "cursor-leader:toggle": {
          description: "Add a cursor at the leader, or drop the one already there.",
          didDispatch: (e) => {
            this.event(e);
            this.toggle();
          },
        },
        "cursor-leader:previous": {
          description: "Make the previous cursor the leader.",
          didDispatch: (e) => {
            this.event(e);
            this.previous();
          },
        ...
        "cursor-leader:next": {
          description: "Make the next cursor the leader.",
          didDispatch: (e) => {
            this.event(e);
            this.next();
          },
      ...
      lumine.commands.add("lumine-text-editor", {
        "cursor-leader:power-editor": {
          description: "Turn leader mode on or off in this editor alone.",
          didDispatch: (e) => {
            this.powerEditor(e);
          },
        },
        ...
        "editor:consolidate-selections": (e) => {
          this.power(this.event(e), false);
        },
        "cursor-leader:reset": {
          description: "Make the first cursor the leader again.",
          didDispatch: (e) => {
            this.event(e);
            this.reset();
          },
        ...
        "cursor-leader:remove": {
          description: "Delete the leader's own cursor, keeping the rest.",
          didDispatch: (e) => {
            this.event(e);
            this.remove();
          },
        ...
        "cursor-leader:skip-leading-whitespace": {
          description: "Move the leader past the indentation of its line.",
          didDispatch: (e) => {
            this.event(e);
        ...
        "cursor-leader:expand-over-word": {
          description: "Grow the leader's selection to the whole word.",
          didDispatch: (e) => {
            this.event(e);
            this.expandOverWord();
          },
        ...
        "cursor-leader:expand-over-line": {
          description: "Grow the leader's selection to the whole line.",
          didDispatch: (e) => {
            this.event(e);
            this.expandOverLine();
          },
    ...
    this.disposables.add(
      ...
      lumine.config.observe("cursor-leader.cursorDecoration", (value) => {
        this.cursorDecoration = value;
      }),
      lumine.config.observe("cursor-leader.decorationTime", (value) => {
  ...
  event(e) {
    ...
    // menu dispatches at whatever holds focus. Mini editors deliberately
    // qualify, because leader commands act on the editor that owns the event.
    return this.update(
      lumine.workspace.getTextEditorForElement(this.target) ??
  ...
  watchEditor(editor) {
    ...
    const state = {
      ...
      originalProperties: new Map(
        PATCHED_EDITOR_PROPERTIES.map((property) => [
          property,
          Object.getOwnPropertyDescriptor(editor, property),
        ]),
      ),
    };
    ...
    editor.cursorHighlight = () => {
      this.destroyCursorDecoration(editor, state);
      if (!this.cursorDecoration || editor.cursors.length <= 1) {
        return;
      }
      const marker = editor.getSuperCursor().getMarker();
      ...
      if (this.decorationTime) {
        state.decorationTimeout = setTimeout(() => {
          if (state.decoration === decoration) {
            ...
            if (editor.cursorDecoration === decoration) editor.cursorDecoration = null;
          }
          decoration.destroy();
        }, this.decorationTime);
      }
  ...
  destroyCursorDecoration(editor, state) {
    if (state.decorationTimeout !== null) {
  ...
  unwatchEditor(editor) {
    ...
    for (const [property, descriptor] of state.originalProperties) {
      if (descriptor) {
        Object.defineProperty(editor, property, descriptor);
      } else {
        delete editor[property];
      }
    }
  ...
  deactivate() {
    ...
    for (const editor of Array.from(this.editorStates.keys())) {
      this.unwatchEditor(editor);
    }
  ...
  powerEditor(e) {
    const element = e.target.closest("lumine-text-editor");
  ...
  activateStatusBar() {
    ...
    this.tooltipDisposable = lumine.tooltips.add(this.statusBarItem, {
  ...
  createStatusBarItem() {
    ...
    element.classList.add("cursor-leader-status");
